...
/*
 * n.b. L1_CACHE_BYTES is not define in barebox's arm cache.h We manually
 * define it here to be 64, which is the largest value it can take based on the
 * possible options in the Linux kernel config.
 * We could theoretically define it smaller, since we dont care about cache
 * alingment, but it is unknown what side-effect that would have.
 */
